...
function BaseLoaderScene(providedCamera, shouldAddLights, shouldRotate, updateMesh) {
  ...
  /**
   * @description 增加一个一次性可以加载多个模型的渲染接口
   * @create ED.J.Yan 2019-19-14 创建
   * @param {objs} meshes 一个模型对象
   */
  this.addMeshes = async function (meshes) {
   
      
      for( obj in meshes ){
        await self.scene.add(meshes[obj]);
        self.stats.update();        
      }
